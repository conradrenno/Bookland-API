package com.devrenno.bookland.payments.architecture;
...
/**
 * Enforces the 4-layer Clean Architecture of the payments module. Domain, Application and Adapters
 * must stay framework-free (Lombok is source-only). Only Infrastructure may touch Spring / JPA / Jackson.
 */
@AnalyzeClasses(packages = "com.devrenno.bookland.payments", importOptions = ImportOption.DoNotIncludeTests.class)
class ArchitectureRulesTest {
    ...
    static final ArchRule domain_is_framework_free =
            noClasses().that().resideInAPackage("..payments.domain..")
                    .should().dependOnClassesThat().resideInAnyPackage(FRAMEWORK_PACKAGES);

    @ArchTest
    static final ArchRule application_is_framework_free =
            noClasses().that().resideInAPackage("..payments.application..")
                    .should().dependOnClassesThat().resideInAnyPackage(FRAMEWORK_PACKAGES);

    @ArchTest
    static final ArchRule adapters_are_framework_free =
            noClasses().that().resideInAPackage("..payments.adapters..")
                    .should().dependOnClassesThat().resideInAnyPackage(FRAMEWORK_PACKAGES);

    @ArchTest
    static final ArchRule dependencies_point_inward = layeredArchitecture()
            .consideringOnlyDependenciesInLayers()
            .layer("Domain").definedBy("..payments.domain..")
            .layer("Application").definedBy("..payments.application..")
            .layer("Adapters").definedBy("..payments.adapters..")
